...
def plot_comparacao_cra(df):
    plot_boxplot(x='FORMA_INGRESSO_SIMPLES', y='CRA', data=df,
...
def plot_correlacao_cra(df):
    ...
    sns.heatmap(df[['CRA', 'IDADE']].corr(), annot=True, cmap='coolwarm', fmt=".2f")
    plt.title('Correlação entre CRA e Idade')
    salvar_grafico('correlacao_cra_idade')


def plot_evasao_cra(df):
    plot_boxplot(x='STATUS_EVASAO', y='CRA', data=df, titulo='CRA por Status de Evasão', xlabel='Status de Evasão',
                 ylabel='CRA')
